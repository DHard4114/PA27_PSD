# Final Project PSD PA27 - Sistem Kendali Robot Arm pada VHDL

## 1. Latar Belakang
Proyek ini bertujuan untuk mengimplementasikan sistem kendali robot arm menggunakan VHDL (VHSIC Hardware Description Language), sebuah bahasa yang digunakan untuk menggambarkan sistem digital yang memungkinkan desain hardware untuk diuji dan diterapkan pada FPGA (Field Programmable Gate Array). Robot arm adalah perangkat keras yang digunakan dalam berbagai aplikasi industri, seperti pengangkatan, perakitan, dan pengelolaan objek. Sistem kendali robot arm ini bertujuan untuk memungkinkan robot beroperasi secara otomatis, dengan kemampuan untuk menavigasi, mengambil, dan melepaskan objek ke lokasi yang ditentukan.

Desain ini mengandalkan beberapa teknik utama dalam VHDL, seperti Microprogramming untuk modularitas, Finite State Machine (FSM) untuk manajemen status operasional robot, dan penggunaan Decoder untuk memproses input koordinat objek dan target. Pendekatan ini memungkinkan desain sistem yang fleksibel, efisien, dan mudah diuji. Sistem ini juga dapat dikembangkan lebih lanjut untuk aplikasi yang lebih kompleks, seperti integrasi sensor atau interaksi dengan perangkat lain.

## 2. Bagaimana Sistem Bekerja

### 2.1 Struktur Sistem
Sistem ini terdiri dari beberapa modul utama yang saling terintegrasi untuk menghasilkan fungsionalitas robot arm. Setiap modul memiliki tugas tertentu dalam proses pengoperasian robot arm, seperti menerima input, melakukan perhitungan navigasi, dan mengelola status sistem.

**Modul Utama dalam Sistem:**

- **Top-Level Entity**  
  Modul ini berfungsi untuk menghubungkan dan mengelola interaksi antara semua modul lainnya, menerima input dari luar dan mengendalikan sinyal output.

- **Input Decoder**  
  Modul ini bertugas untuk memproses data koordinat yang diterima sebagai input gabungan (48-bit) untuk memisahkan koordinat objek dan target, kemudian mendistribusikan informasi tersebut ke modul navigasi.

- **Navigator**  
  Modul ini melakukan navigasi robot arm dengan membandingkan posisi saat ini dengan posisi target dan objek. Modul ini memandu robot untuk bergerak menuju posisi yang diinginkan dengan mengaktifkan motor.

- **Finite State Machine (FSM)**  
  Modul ini mengatur status dan transisi sistem berdasarkan peristiwa yang terjadi. FSM mengontrol langkah-langkah operasi robot, seperti bergerak ke objek, menggenggam objek, bergerak ke target, dan melepaskan objek.

### 2.2 Alur Kerja Sistem Berdasarkan FSM
FSM mengelola alur kerja robot arm melalui beberapa status:

- **IDLE**: Menunggu sinyal perintah untuk mulai bergerak.
- **NAV_TO_OBJ**: Robot bergerak menuju objek.
- **GRIP_OBJ**: Robot menggenggam objek setelah mencapai koordinat objek.
- **NAV_TO_TGT**: Robot bergerak menuju target.
- **RELEASE_OBJ**: Robot melepaskan objek setelah mencapai target.
- **ERROR**: Sistem berada dalam keadaan error (misalnya, kegagalan kalibrasi atau error gerakan).

**Diagram Alur FSM**

```mermaid
stateDiagram-v2
    [*] --> IDLE : Reset (rst = '1')
    
    state IDLE {
        [*] --> NAV_TO_OBJ : Sinyal Start (start = '1')
        motor_en = '0'
        gripper_open = '0'
        pos_reached = '0'
    }
    note right of IDLE
        Kondisi awal setelah reset
        Semua output default
        Menunggu sinyal start
    end note
    
    IDLE --> NAV_TO_OBJ : start = '1'
    
    state NAV_TO_OBJ {
        [*] --> MOVE_TO_OBJ : Aktifkan motor
        motor_en = '1'
        gripper_open = '0'
        pos_reached = '0'
    }
    note right of NAV_TO_OBJ
        Aktifkan motor navigasi
        Gerakkan ke koordinat objek
        Tunggu pos_reached = '1'
    end note

    NAV_TO_OBJ --> GRIP_OBJ : pos_reached = '1'
    NAV_TO_OBJ --> ERROR : Error Terjadi
    
    state GRIP_OBJ {
        [*] --> OPEN_GRIPPER : Buka gripper
        motor_en = '0'
        gripper_open = '1'
        pos_reached = '1'
    }
    note right of GRIP_OBJ
        Buka gripper
        Siapkan genggaman
        Matikan motor
    end note
    
    GRIP_OBJ --> NAV_TO_TGT : Objek Siap Digengam
    GRIP_OBJ --> ERROR : Error Terjadi
    
    state NAV_TO_TGT {
        [*] --> MOVE_TO_TGT : Aktifkan motor
        motor_en = '1'
        gripper_open = '1'
        pos_reached = '0'
    }
    note right of NAV_TO_TGT
        Aktifkan motor navigasi
        Gerakkan ke koordinat target
        Tunggu pos_reached = '1'
    end note
    
    NAV_TO_TGT --> RELEASE_OBJ : pos_reached = '1'
    NAV_TO_TGT --> ERROR : Error Terjadi
    
    state RELEASE_OBJ {
        [*] --> OPEN_GRIPPER_RELEASE : Lepaskan objek
        motor_en = '0'
        gripper_open = '0'
        pos_reached = '1'
    }
    note right of RELEASE_OBJ
        Lepaskan objek di target
        Matikan motor
        Gripper terbuka
    end note
    
    RELEASE_OBJ --> IDLE : Objek Dilepas
    RELEASE_OBJ --> ERROR : Error Terjadi
    
    state ERROR {
        [*] --> HANDLE_ERROR : Tangani error
        motor_en = '0'
        gripper_open = '0'
        pos_reached = '0'
    }
    note right of ERROR
        Tangani kondisi error
        Memungkinkan reset ulang
        Cek komponen terkait error
        Reset komponen atau seluruh sistem
        Log kesalahan
        Notifikasi pengguna
    end note
    
    IDLE --> ERROR : Sistem Error Terdeteksi
    ERROR --> IDLE : Reset Sistem
```

### 2.3 Diagram Alur FSM
FSM berfungsi untuk memastikan bahwa robot arm selalu berada dalam satu status yang jelas pada setiap waktu. Dengan memeriksa kondisi seperti `pos_reached` atau `error_flag`, robot dapat berpindah status sesuai kebutuhan.

## 3. Cara Penggunaan

### 3.1 Tabel Input dan Output Harapan
Tabel berikut menjelaskan cara menggunakan sistem kendali robot arm ini melalui input dan output yang diharapkan pada setiap langkah.

| No. | Input Testbench | Deskripsi | Output Harapan |
|-----|-----------------|-----------|----------------|
| 1   | rst = '1'       | Reset sistem | Semua sinyal output (pos_reached, gripper_open, motor_en, dll.) direset ke nilai default. FSM berada di state IDLE. |
| 2   | rst = '0'       | Reset selesai | Sistem siap menerima input. FSM tetap di state IDLE, menunggu sinyal start. |
| 3   | input_data = "000000000001010000000101000001100011001100" <br> start = '1' | Input data koordinat: Objek (10, 20, 30), Target (50, 40, 60). Mulai navigasi. | FSM berpindah dari IDLE ke NAV_TO_OBJ. Navigasi robot dimulai menuju koordinat objek (10, 20, 30). Output: motor_en = '1', pos_reached = '0'. |
| 4   | start = '0'     | Navigasi robot berjalan. | Robot terus bergerak ke objek. FSM tetap di NAV_TO_OBJ hingga `pos_reached = '1` untuk objek (10, 20, 30). Output: x_out, y_out, z_out menuju (10, 20, 30). |
| 5   | FSM mencapai koordinat objek (10, 20, 30) <br> pos_reached = '1' | Objek tercapai | FSM berpindah ke GRIP_OBJ. Output: gripper_open = '1' untuk mengambil objek, motor_en = '0'. |
| 6   | FSM berpindah ke NAV_TO_TGT: Robot bergerak menuju koordinat target (50, 40, 60). | Navigasi menuju target | FSM berada di state NAV_TO_TGT. Output: motor_en = '1', pos_reached = '0'. x_out, y_out, z_out menuju target (50, 40, 60). |
| 7   | FSM mencapai koordinat target (50, 40, 60) <br> pos_reached = '1' | Target tercapai | FSM berpindah ke RELEASE_OBJ. Output: gripper_open = '0' untuk melepaskan objek, motor_en = '0'. |
| 8   | FSM kembali ke state IDLE setelah objek dilepaskan. | Sistem menunggu perintah baru | FSM kembali ke state IDLE. Semua sinyal output kembali ke nilai default: motor_en = '0', gripper_open = '0', pos_reached = '0'. |

### 3.2 Output 7-Segment Display
Status FSM akan ditampilkan pada 7-segment display dengan format berikut:

| State        | Tampilan 7-Segment |
|--------------|--------------------|
| IDLE         | 000                |
| NAV_TO_OBJ   | 001                |
| GRIP_OBJ     | 010                |
| NAV_TO_TGT   | 011                |
| RELEASE_OBJ  | 100                |
| ERROR        | 111                |