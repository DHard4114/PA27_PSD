...
ENTITY Navigator IS
    PORT (
        ...
        pos_reached : OUT STD_LOGIC -- Position reached flag
    );
END Navigator;

ARCHITECTURE Behavioral OF Navigator IS
    SIGNAL x_pos, y_pos, z_pos : INTEGER RANGE 0 TO 999; -- Integer untuk posisi robot
    SIGNAL target_reached : STD_LOGIC;
BEGIN
    -- Clocked process to update position
    PROCESS (clk, rst)
    BEGIN
        IF rst = '1' THEN
        ...
        ELSIF rising_edge(clk) THEN
            IF start = '1' THEN
                -- Update position towards target (simulating movement)
    ...
    -- Output current positions (integer)
    current_x <= x_pos;
    current_y <= y_pos;
    current_z <= z_pos;

    -- Output position reached status
    pos_reached <= target_reached;
END Behavioral;